...
from fasthtml.common import A, H3, H4, Img, Button, Div, Input, Label, P, Script, Span
...
def _render_see_also(entries, title):
    ...
    for entry in _ensure_list(entries):
        ...
        if url:
            items.append(A(label, href=url, target="_blank", rel="noreferrer", cls="text-xs font-semibold uppercase tracking-[0.3em] text-indigo-600 dark:text-indigo-300"))
    if not items:
        return None
    return Div(
        H4(title, cls="text-xs font-bold uppercase tracking-[0.3em] text-slate-400"),
...
def _render_providers(entries):
    ...
    for provider in providers:
        ...
        if not name and not homepage:
            continue
        link = A("Sito", href=homepage, target="_blank", rel="noreferrer", cls="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-500") if homepage else None
        rows.append(
            Div(
...
def info_tab_content(meta, total_pages, manifest_json, page_idx, doc_id, library):
    ...
    canvas_preview = (
        Div(Img(src=thumbnail_url, alt=canvas_label or title, cls="w-full h-52 object-cover rounded-xl"), cls="mt-4 overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700")
        if thumbnail_url
        else None
    )
    document_rows = [
        ...
        info_row("Manifesto ufficiale", A("Apri manifest", href=manifest_link, target="_blank", rel="noreferrer") if manifest_link else "N/D"),
