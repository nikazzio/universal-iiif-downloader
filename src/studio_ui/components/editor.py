...
def ocr_controls(doc_id: str, library: str, current_page: int, has_transcription: bool = False) -> Div:
    ...
    return Div(
        ...
        Div(
            Label(
                "Engine OCR:", for_="ocr-engine", cls="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
            ),
            Select(
                ...
                cls=(
                    ...
                    "rounded bg-white dark:bg-gray-800 dark:text-white"
                ),
            ),
            cls="mb-4",
        ),
        ...
        Div(
            Label(
                "Modello HTR:", for_="ocr-model", cls="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"
            ),
            Select(
        ...
        Form(
            ...
            Button(
                ...
                type="submit",
                cls=(
                    "w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 "
                    "dark:hover:bg-indigo-600 text-white rounded transition-all hover:scale-105"
                ),
            ),
        ...
        Div(
            ...
            Button(
                ...
                cls=(
                    "w-full py-2 px-4 bg-purple-600 hover:bg-purple-700 dark:bg-purple-500 "
                    "dark:hover:bg-purple-600 text-white rounded transition"
                ),
            ),
            cls="mb-4",
        ),
        ...
        Script("""
            ...
            function confirmBatchOCR() {
                if (confirm(
                    'Confermi di voler eseguire OCR su tutte le pagine? ' +
    ...
    if status == "success":
        return Div(
            ...
            cls=(
                ...
                "text-green-700 dark:text-green-200 px-4 py-3 rounded flex items-center",
            ),
        )
    elif status == "error":
        return Div(
            Span("❌", cls="text-2xl mr-2"),
            Div(Div("Errore OCR", cls="font-medium"), Div(message, cls="text-xs mt-1"), cls="flex-1"),
            cls=(
                "bg-red-100 dark:bg-red-900 border border-red-400 dark:border-red-600 "
                "text-red-700 dark:text-red-200 px-4 py-3 rounded flex items-center",
            ),
        )
